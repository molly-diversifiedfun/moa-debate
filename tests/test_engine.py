...
async def test_call_model_handles_failure():
    ...
    with patch("moa.orchestrator.acompletion", side_effect=Exception("API Error")), \
...
async def test_call_model_timeout():
    ...
    with patch("moa.orchestrator.acompletion", side_effect=slow_response), \
         patch("moa.health.should_skip", return_value=None):
        result = await call_model(
            GEMINI_FLASH,
...
async def test_call_model_returns_real_cost():
    ...
    with patch("moa.orchestrator.acompletion", return_value=mock_response), \
         patch("moa.health.should_skip", return_value=None):
        result = await call_model(
            GEMINI_FLASH,
            [{"role": "user", "content": "test"}],
